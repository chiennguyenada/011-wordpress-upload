...
def upload_product(product_name, short_description, description, focus_keyword, rank_math_description, avatar_info, combined_keywords, categories, tags):
    # Kiểm tra xem sản phẩm đã tồn tại hay chưa trước khi thử tải lên
    existing_product = check_product_exists(focus_keyword)
    
    if existing_product:
        print(f"Sản phẩm với SKU '{focus_keyword}' đã tồn tại (ID: {existing_product['id']}).")
        print(f"Vui lòng sử dụng tùy chọn --skip-exist hoặc --force-update để xử lý sản phẩm đã tồn tại.")
        print(f"Hoặc sử dụng chế độ cập nhật hình đại diện cho sản phẩm đã tồn tại.")
        return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 201:
                ...
                # Trường hợp SKU đã tồn tại nhưng không phát hiện được trong bước kiểm tra trước
                print("Lỗi: SKU đã tồn tại. Vui lòng sử dụng tùy chọn --skip-exist hoặc --force-update.")
                return False
            else:
...
def check_and_update_or_create(focus_keyword, product_name, short_description, description, 
                               rank_math_description, avatar_info, combined_keywords, categories, tags, force_update=False):
    """
    Kiểm tra xem sản phẩm với SKU đã tồn tại chưa, nếu có thì cập nhật hoặc bỏ qua, nếu không thì tạo mới
    
    Args:
        focus_keyword: SKU/mã sản phẩm
        product_name, short_description, description,...: Các thông tin sản phẩm
        force_update: Nếu True, tự động cập nhật sản phẩm đã tồn tại mà không hỏi người dùng
        
    Returns:
        bool: True nếu đã xử lý (tạo mới hoặc cập nhật), False nếu bỏ qua
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if products and len(products) > 0:
                # Sản phẩm đã tồn tại, quyết định cập nhật hoặc bỏ qua
                product_id = products[0]['id']
                print(f"Tìm thấy sản phẩm có SKU '{focus_keyword}' với ID: {product_id}, URL: {products[0].get('permalink', 'Không có URL')}")
                
                if force_update:
                    print("Chế độ force update được kích hoạt - tự động cập nhật sản phẩm")
                ...
                # Không tìm thấy sản phẩm, tạo mới
                return upload_product(product_name, short_description, description, focus_keyword, 
                                     rank_math_description, avatar_info, combined_keywords, categories, tags)
        else:
...
def check_product_exists(sku):
    """
    Kiểm tra xem sản phẩm đã tồn tại trong hệ thống bằng SKU hay chưa.
    
    Args:
        sku (str): Mã SKU cần kiểm tra
        
    Returns:
        dict or None: Trả về thông tin sản phẩm nếu đã tồn tại, None nếu không tồn tại
    """
    try:
        headers = {
            "Content-Type": "application/json"
        ...
        params = {
            "consumer_key": CONSUMER_KEY,
            "consumer_secret": CONSUMER_SECRET,
            "sku": sku  # Tìm kiếm theo SKU
        }
        
        # Gửi request để kiểm tra
        response = requests.get(
            f"{WP_URL}/wc/v3/products",
            headers=headers,
            params=params,
        ...
        if response.status_code == 200:
            ...
            if products and len(products) > 0:
                ...
                print(f"Đã tìm thấy sản phẩm với SKU: {sku}, ID: {products[0]['id']}")
                return products[0]
            else:
                print(f"Không tìm thấy sản phẩm với SKU: {sku}")
                return None
        else:
            print(f"Lỗi khi kiểm tra sản phẩm: {response.status_code}")
            print(response.text)
            return None
    except Exception as e:
        print(f"Lỗi khi gửi request kiểm tra sản phẩm: {str(e)}")
        return None